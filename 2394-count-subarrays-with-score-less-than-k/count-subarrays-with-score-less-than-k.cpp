...
public:
    long long countSubarrays(vector<int>& nums, long long k) {
        ...
        while(end<nums.size()){
            sum+=nums[end];
            score=sum*(1+end-start); // 1+(end-start) will give length of window
            ...
            while(score>=k && start<=end){
                ...
                score=sum*(1+end-start); //1+end-start gives new length of window
                
            }

            count+=1+(end-start); //size of windpw will tells about how many subarr
            end++;
        }
        return count;
    }
